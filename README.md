...
- 루트의 [DESIGN.md](DESIGN.md) 를 기준으로 화면 톤을 맞춥니다.
- 화면 구조와 UX 흐름은 `stitch_ai` 시안을 먼저 참고합니다.
- 현재 디자인 방향은 `CorpScale` 계열의 내부 업무 도구 톤을 참고합니다.
- 핵심 기준은 네이비/그레이 중심 색상, 세리프 제목, 절제된 반경, 촘촘한 업무형 레이아웃입니다.
- 시안 참고 파일은 `stitch_ai/` 에 있을 수 있지만, 실제 디자인 기준 문서는 `DESIGN.md` 입니다.
