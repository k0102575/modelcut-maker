...
| 생성 품질 | `generation_mode` | 강함 | `fast / balanced / quality`, 기본값은 `fast` |
| 사진 비율 | `aspect_ratio` | 강함 | 화면 선택 없이 `3:4` 고정 |
...
| 모델 성별 | `prompt` 문장 안 여성 모델 힌트 | 약함 | 화면 선택 없이 여성 고정 |
...
| 사진 비율 | `aspect_ratio` | 강함 | 화면 선택 없이 `3:4` 고정 |
| 생성 품질 | `generation_mode` | 강함 | 속도/비용/디테일 차이 큼, 기본값은 `fast` |

### 약한 정보

| 화면 입력 | API 전달 방식 | 영향도 | 메모 |
| --- | --- | --- | --- |
| 모델 성별 | `prompt`에 합쳐짐 | 약함~중간 | 화면 선택 없이 여성 고정 |
| 나이대 | `prompt`에 합쳐짐 | 약함~중간 | 전용 파라미터 아님 |
...
| 생성 품질 | `generation_mode` | 강함 | 속도/비용/디테일 차이, 기본값은 `fast` |

### 약한 정보

| 화면 입력 | API 전달 방식 | 영향도 | 메모 |
| --- | --- | --- | --- |
| 사람 설명 | `prompt` | 약함~중간 | 나이대/분위기 힌트 |
...
- 참고 얼굴 사진
- 생성 품질
- 사진 비율: 현재 선택 없이 `3:4` 고정

## 7. 지금 기준으로 결과 영향이 상대적으로 약한 입력

- 카테고리
- 모델 성별: 현재 선택 없이 여성 고정
- 나이대
- 보이는 범위
- 촬영 방향
- 배경 느낌
